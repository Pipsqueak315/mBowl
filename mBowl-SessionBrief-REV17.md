...
**Phase:** Phase 19 complete -- Log QOL, frame editing, share card, game decay, goals, restore
...
- **Shared types:** src/types.ts is the canonical source for ThrowEntry, FrameData, GameEntry, Session, Ball, Settings, DraftData. Do not redefine these locally. Settings includes targetAverage and targetSeries (added Phase 19).
- **Storage layer:** src/storage.ts (TypeScript). KEYS object exported. All functions fully typed. No any returns.
- **Log Frames QOL:** Max Available Score (Live+Pins only), Running Series Total (game 2+), Strike Streak badge with spring animation. Frame strip cells are tappable to jump to any frame.
- **Auto-export:** src/backup.ts exports writeBackup() and restoreBackup(). writeBackup() called non-blocking (void) after every meaningful write. restoreBackup() reads mBowl-backup.json, validates, atomically restores all 4 keys. Restore UI is in SettingsContent.tsx (destructive alert confirmation).
...
- **Frame editing from History:** Dismiss-and-return pattern. EditSessionModal closes → history.tsx useEffect pushes to log-frames → useFocusEffect return reads FRAME_RESULT_KEY, reopens modal with updated data. pendingFrameEditRef + shouldPushFramesRef guard the flow.
- **Mini scorecard on Log tab:** GameRow shows inline frame grid + mini pin deck (when frames exist). Score override alert fires on blur when manually entered score differs from calculatedScore.
...
- **Goals in Settings:** Target Average (0-300) and Target Series (0-900) stored as targetAverage / targetSeries in Settings. Stats tab hero card and High Series card show color-coded delta vs goal.
...
- Frame data shows read-only notice ("Frame data cannot be edited") with lock icon when frames exist -- replaced by Edit Frames button in Phase 19
...
- If pin data exists: cards shown with count (xN), conversion % (color-coded >=80/60/below), teal frequency bar (3px, scaled to most frequent leave in the current filtered view)
...
### Phase 19 -- Log QOL, Frame Editing, Share Card, Game Decay, Goals, Restore
**Completed:** April 3, 2026

**A1 -- Ball picker race condition fix:**
- `openBallPicker` in log.tsx now calls `readBalls()` immediately before opening the picker
- Previously balls only loaded on mount -- stale if user added a ball in Settings mid-session

**B1 -- Frame strip navigation:**
- FrameStripItem cells in log-frames.tsx are now `TouchableOpacity` (was `View`)
- Tapping any frame cell jumps directly to that frame with haptic (Light)
- `isCurrent` highlight always active (was gated on `!allComplete`)

**B2 -- Inline mini scorecard on Log tab:**
- `GameRow` shows a frame grid (10 cells) + mini pin deck below score input when `game.frames` exists
- Frame grid: each cell shows throw notation chips, strike chips in teal; 10th frame wider
- Mini pin deck: 4-row layout mirrors full PinDeck, shows last frame's pinsStanding (or infer from completed frames)
- New local components: `FrameGrid`, `MiniPinDeck`, `MINI_PIN_ROWS`

**B3 -- Edit Frames button + score override warning:**
- Log tab game row button label changes to "Edit Frames" when `game.frames` already exists
- `calculatedScore` field on `Game` type stores the score returned from log-frames
- Score TextInput `onEndEditing` fires when user manually changes score: if differs from `calculatedScore` by more than 0, shows Alert("Override score?") explaining the discrepancy
- `onLogFrames` callback passes `initialFrames: JSON.stringify(game.frames)` to log-frames when frames exist

**B4 -- Edit frame data from History (EditSessionModal):**
- "Edit Frames" / "Log Frames" button in each game row of EditSessionModal (replaces lock icon/notice)
- Dismiss-and-return navigation: modal closes → history.tsx useEffect detects `shouldPushFramesRef` → pushes to log-frames with game index + initialFrames + mode params → on focus return, useFocusEffect reads FRAME_RESULT_KEY → reopens modal with updated session data
- Guards: `pendingFrameEditRef` stores `{ sessionId, gameIndex }`, `shouldPushFramesRef` triggers the push
- `active` flag in useFocusEffect load() prevents setState-after-unmount

**B5 -- Pins mode default in Post-Game:**
- log-frames.tsx `inputMode` state initializes to `'pins'` regardless of mode
- Mode useEffect: `setInputMode('pins')` always (was conditional on live/post-game)

**C1 -- Game-by-game performance decay card in Stats:**
- New "By Game Number" card between "By Ball" and "Common Leaves"
- Shows average score for games 1, 2, 3 across all filtered sessions with at least that many games
- `buildGameByGameStats()` function + `GamePositionStat` interface
- `gameByGameStats` useMemo keyed on `filtered`
- Average color-coded by standard thresholds; sample count shown in dim text below

**C2 -- Share card on History expanded cards:**
- Share button appears below session notes in expanded SessionCard
- `ShareCardView` component renders off-screen (position absolute, left: -10000): date badge, session type, scores with color coding, frame grid thumbnail, stats row, watermark
- `captureRef()` from react-native-view-shot captures the off-screen view as PNG
- `shareAsync()` from expo-sharing opens iOS share sheet
- `sharing` state shows ActivityIndicator during capture; iOS only (Platform.OS guard)
- react-native-view-shot and expo-sharing added to dependencies

**C3 -- Goal tracking in Settings + Stats delta indicators:**
- "GOALS" section in SettingsContent.tsx: Target Average (TextInput, 0-300) and Target Series (TextInput, 0-900)
- Saves on blur to `mbowl_settings_v1` as `targetAverage` / `targetSeries` (number or null)
- `Settings` type in src/types.ts updated with `targetAverage?: number | null` and `targetSeries?: number | null`
- Stats hero card: `avgGoalDelta` useMemo shows "+X above goal" (green) / "X below goal" (red) / "On goal" (teal) below average number when goal is set
- Stats High Series card: `seriesGoalDelta` useMemo shows same delta pattern vs target series
- SettingsContent.tsx: removed local `type Settings` and `type Ball` -- now imports from `@/src/types` (canonical types only)

**C4 -- Data restore from backup in Settings:**
- "Restore from Backup" button at bottom of SettingsContent (red text, destructive)
- Confirmation Alert: "This will overwrite all current data. Are you sure?"
- `restoreBackup()` added to src/backup.ts: reads mBowl-backup.json, validates JSON + shape, atomically restores all 4 AsyncStorage keys via multiSet
- Returns `{ success: boolean; error?: string }` -- shows Alert with result
- `KEYS` imported from storage.ts in backup.ts

**Production audit (Phase 19):**
- MUST FIX: `Platform.OS !== 'ios'` guard fixed in ShareCardView (was `!Platform.OS === 'ios'` which always evaluated false)
- MUST FIX: `active` flag added to useFocusEffect load() in history.tsx (setState-after-unmount)
- MUST FIX: `active` flag added to readBalls useEffect in EditSessionModal.tsx
- SHOULD FIX: Stats goal deltas extracted from IIFEs into proper `avgGoalDelta` and `seriesGoalDelta` useMemos
- TSC baseline error fixed: `components/ui/icon-symbol.tsx` stub created (TypeScript couldn't resolve `.ios.tsx` -- pre-existing, not introduced in Phase 19)

**Deployed:**
- Preview branch: update group 740a7ec2-cef0-4fd9-83bb-f5504242708f
- Production branch: update group 02cc37bb-4754-4800-b103-15ecf49ce68c
- TSC: Clean (zero errors confirmed)
...
- New iPhone or iCloud restore → Phase 17 backup travels via iCloud Drive. Redo Phase 1-3 install, import JSON backup to restore all session data. Restore UI now in Settings (C4).
