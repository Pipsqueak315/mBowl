...
Tell Claude Code at the top of each session: "Phase X — [what you're working on today]."  
Claude reads CLAUDE.md and mBowl-SPEC.md for full context. This brief tracks current state only.

---

## ⚠️ Critical Project Notes (Read Before Phase 3)
...
- `app/_layout.tsx` is the root layout (equivalent of App.js). Seed logic was wired here in Phase 2.
- `app/(tabs)/_layout.tsx` controls the bottom tab bar.
- React Navigation is already included as a dependency via Expo Router — do NOT reinstall it.
- Phase 3 prompt must reference Expo Router file structure, not manual navigator setup.
...
- `src/balls.js` created — 9-ball roster, weakest → strongest, all `active: true`
...
**Goal:** Replace the default Expo starter UI with mBowl's 4-tab shell. No screen content yet — just correct structure, icons, colors, and navigation wired up.

**Claude Code prompt to use:**
```
Read CLAUDE.md and mBowl-SPEC.md before writing any code.

I'm on Phase 3 — Navigation Shell. This is an Expo Router project. The tab structure lives in app/(tabs)/.

Complete the following:

1. Replace app/(tabs)/_layout.tsx with mBowl's 4-tab config:
...
5. Delete any default Expo starter files that are no longer needed
   (index.tsx, explore tab, etc.) — clean slate.
...
**Done when:** 4 tabs working on iPhone, teal active icon, gear opens modal, no Expo starter content.
